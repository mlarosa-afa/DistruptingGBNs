...
def adaGrad(gradient, constraint, curr_pos, learn_rate=0.001, v = 0, epsilon = 0.00001):
    direction = gradient(curr_pos) / np.linalg.norm(gradient(curr_pos))
    step = (learn_rate / sqrt(v + epsilon)) * direction
    v_t_1 = v + pow(np.linalg.norm(gradient(curr_pos)), 2)
    return check_gd_bounds(constraint, curr_pos, step), v_t_1

def RMSProp(gradient, constraint, curr_pos, learn_rate= 0.001, v = 0, beta = .9, epsilon = .00001):
    direction = gradient(curr_pos) / np.linalg.norm(gradient(curr_pos))
    step = ((learn_rate / sqrt(v + epsilon)) * gradient(curr_pos))
    v_t_1 = (beta * v) + ((1-beta) * pow(np.linalg.norm(gradient(curr_pos)), 2))
    return check_gd_bounds(constraint, curr_pos, step), v_t_1

def adam(gradient, constraint, curr_pos, learn_rate= 0.001, t = 0, v = 0, m = 0, beta_1 = .9, beta_2 = .999, epsilon = 0.00001):
